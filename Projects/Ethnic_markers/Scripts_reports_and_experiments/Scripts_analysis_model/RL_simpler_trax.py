...
def RL_simpler_trax(l,e):
    N=20
    T=40
    sim=1000
    h=0
    pdif=2 #number of different probabilities (2xbehaviours)
    prob2 = 1.0#Probability of learner in a population
    prob3 = 0.0	 #Probability of conformist
    bins = 40   
    ...
    coord_dist_global = [0]*bins
    coord_dist_global_2 =[0]*bins
    prob_dist_global = [0]*bins
    prob_err_global = [0]*bins
    for s in range(sim):
        ##Initial conditions
        for i in range(N):
        ...
        coord_dist_global = [x+y for  x,y in zip(coord_dist_global,coord_dist)]
        coord_dist_global_2 = [x+y**2 for  x,y in zip(coord_dist_global_2,coord_dist)]
        prob_dist_global = [x+y for x,y in zip(prob_dist_global,prob_dist)]
        prob_err_global = [x+y for x,y in zip(prob_err_global,prob_err)]
    coord_dist_global = [x/sim for x in coord_dist_global]
    coord_dist_global_2 = [x/sim for x in coord_dist_global_2]
    prob_dist_global = [x/sim for x in prob_dist_global]
    coord_dist_global_2 = [np.sqrt(x-y**2) for x,y in zip(coord_dist_global_2,coord_dist_global)]
    prob_err_global = [x/sim for x in prob_err_global]
    
    final_results = [coord_dist_global,
                     coord_dist_global_2,
                     prob_dist_global,
                     prob_err_global]
