...
start=time.time()
N=10000             #Número de individuos por grupo
Ng=2  
tfinal=1200*N
...
beta=0.1          #Frecuencia da migración (en unidades de poboación)            
...
for i in range(0,Ng):
    ...
    plt.ylabel('Agents')
    plt.legend()
    fig.savefig('EM2Fig1G'+str(i+1)+'b='+str(beta)+'d='+str(delta)
...
for i in range(0,Ng):
    ...
    plt.ylabel('Payoff') 
    plt.legend()
    fig.savefig('EM2Fig1P'+str(i+1)+'b='+str(beta)+'d='+str(delta)
    +'e='+str(e)+'r='+str(r)+'m='+str(m)+'.jpg')
