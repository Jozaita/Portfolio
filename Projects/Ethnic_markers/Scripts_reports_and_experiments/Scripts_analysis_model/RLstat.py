...
### Variables del juego en general, fuera de agentes
### Matriz de pagos, tiempo, simulaciones
...
A[1,1]=1+delta
##Agents variables
## La clase agente necesita características individuales
## comportamiento, marcadores,Aspiración,payoff,e,l,h, vector de probabilidad por categoría, contactos (forma red?,matriz adyacente igual)
## Al inciar un agente con 1 marcador, podemos iniciar dentro del agente:
##          Vector de n probabilidades, que podemos tirar con números aleatorios para empezar.
##          Cada elemento del vector de probabilidad puede tener N-1 componentes, en caso de que haya N opciones para cada categoría.
##          Vector de contactos, que es una fila de matriz de adyacencia externa u otra manera
##          Aspiración, tomada de una distribución externa de aspiraciones 
##          Parámetros e,l,h, globales o de una distribución
##Al final, agente requiere como atributos:
## Distribución de marcadores, matriz de adyacencia, distribución de aspiraciones,l,e,h.
##Hay que definir métodos dentro del agente: 
##          Para cada timestep, se puede seleccionar un agente, coger aleatoriamente un contacto e interaccionar.
##          Coger un contacto aleatoriamente, observar los n marcadores del partner, escoger categoría 
##          Hacer acción proporcional al número aleatorio tirado dentro del vector de probabilidades 
##          Con la acción del partner podemos hacer un método interno que actualice todo, esto es:
##          Con la acción calculamos el pago, con el pago el estímulo, con el estímulo la update
##Para cada timestep: 
##Función externa
## Selección de pareja de agentes, cada agente tiene qué decidir que hacer
## Igual molaba un método interno que cogiera el marcador del partner y sacara qué se va a hace
## A partir de ahí, con lo que se saque de la función se mete en un atributo, para sacarlo fuera
## ver si es igual. Con eso definimos un método que coja el payoff, y actualice todo
##Y al carrer
## A partir de aquí todo sería funciones de plot
e=1
h=0
l=0.05
...
for s in range(sim):
    ...
    for t in range(T):
        if t>3*N:
            print(np.average([agents[i][2]/agents[i][3] for i in range(N)]),np.var([prob[i][0] for i in range(N)]))

        # if np.mod(t,resto/2)==0:
        #     trax2.append(np.sum([agents[i][2]/(resto) for i in range(N)]))
        #     for i in range(N):
        #         agents[i][2] = 0
        ##Termalization
        ...
        for i in couple:
            ...
            #if i == 0:
            st_dist.append(stimulus)
            prob_dist.append(prob[i][action[k]//2])
