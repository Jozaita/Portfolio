...
for j in uasp:
    k3+=1
    histopay,histopay2=np.histogram([agents[i][2] for i in range(N) if agents[i][4]==j],bins=100,range=(0,2*(T/N)))
    civec[k3]+=histopay
    k4=-1
    for u in [0,2]:
        k4+=1
        ...
        if (math.isnan(pearson)==False): 
            corrvec[k3][k4]+=np.arctanh(pearson)
            corrfactor[k3][k4]+=1
civec[-1]+=np.histogram([agents[i][2] for i in range(N)],bins=100,range=(0,2*(T/N)))[0]
k4=-1
for u in [0,2]:
    k4+=1
    ...
    if (math.isnan(pearson)==False):
        corrvec[-1][k4]+=np.arctanh(pearson)
        corrfactor[-1][k4]+=1
