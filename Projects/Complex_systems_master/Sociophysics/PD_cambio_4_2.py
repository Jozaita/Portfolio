 ...
 #For each time step, every agent plays PD with his neighbours'])
   ...
   for t in range(1,tfinal):
      ...
      if t==contador:
       ...
       data2=[values[i] for i in range(0,N)]
       data3=[links[i][0] for i in range(0,len(links))]
       data4=[links[i][1]  for i in range(0,(len(links)))]
       datafinal=list(zip(data1,data2))  
       datafinal2=list(zip(data3,data4)) 
       df = pd.DataFrame(data =datafinal, columns=['Id ', 'Attribute'])
       df2 = pd.DataFrame(data =datafinal2, columns=['Source ', 'Target'])
       #df.insert(df.shape[1],column='Label'+str(t),value=[values[i] for i in range(0,N)],allow_duplicates=True)
       #df2.insert(df2.shape[1],column='Source'+str(t),value=[links[i][0] for i in range(0,len(links))],allow_duplicates=True) 
       #df2.insert(df2.shape[1],column='Target'+str(t),value=[links[i][1] for i in range(0,len(links))],allow_duplicates=True) 
       
      #df.to_csv('Nodosprueba2'+str(t)+'.csv')
      #df2.to_csv('Linksprueba2'+str(t)+'.csv')
