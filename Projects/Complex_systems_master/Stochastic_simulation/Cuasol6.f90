...
call cpu_time(start)
call dran_ini(5000)
open(unit=75,file='Pruebo6.txt',status='replace',action='write')
...
do rho2=0,0
	...
	open(unit=75,file='Pruebo6.txt',status='old',position='append',action='write')
	write(75,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=75)
