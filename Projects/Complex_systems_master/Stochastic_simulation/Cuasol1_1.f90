...
real(dp),dimension(60000)::agemaj,agemin
real(dp)::dran_u,start,al2,al4,al5,a,sumaj,sumin,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin,x


call dran_ini(5000)
open(unit=70,file='Pruebee4_2.txt',status='replace',action='write')
...
do rho2=0,0
	...
	open(unit=70,file='Pruebee4_2.txt',status='old',position='append',action='write')
	write(70,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)finish-start
	close(unit=70)
