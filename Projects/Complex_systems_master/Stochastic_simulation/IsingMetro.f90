...
real(dp)::rm0,rm1,e1,rm,rm2,taum,taue,Temp,corr,pa,u4,mc,mtemp,etemp
call cpu_time(start)
call dran_ini(5000)
open(unit=10,file='IsingL4.txt',status='replace',action='write')
open(unit=15,file='Termalizacion.txt',status='replace',action='write')
open(unit=16,file='Corr.txt',status='replace',action='write')
M0=1000
...
write(*,*) Temp
  ...
  mtemp=(abs(sum(s))/dble(N))
  write(15,'(13(f13.7,5x))')mtemp
  end do
  corr=0.d0
  cm=0.d0
  ce=0.d0
  ...
  do im=1,Measure
     ...
     etemp=0.d0
  do i=1,N
    etemp=etemp-dble(s(i)*(s(n1(i))+s(n2(i))+s(n3(i))+s(n4(i))))/dble(N)
  end do
     write(16,'(1(f13.7,5x))')etemp
     end if 
     end do
