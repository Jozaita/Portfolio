...
integer,parameter:: long=3000
integer(dp),parameter::sim=Num/10
integer(dp)::tray,i,j,cont,a2,rho2,nmax
real(dp)::n,r,xm,xm2,xm4,x,q,m,taumaj,taumin,rho,iprom,ivar,a
...
open(unit=35,file='NVMpagn41.txt',status='new',action='write')
 close(unit=35)	

do a2=1,10
a=dble(a2)*5.0d0*10.0d0**(-3.0d0)
do rho2=1,10
	call cpu_time(start)
	xm=0.0d0
	xm2=0.0d0
	...
	rho=0.40d0+dble(rho2)*10.0d0**(-2.0d0)
	tmax=2*10**(3)	
	do tray=1,sim
	...
	call cpu_time(finish)
	open(unit=35,file='NVMpagn41.txt',status='old',position='append',action='write')
	write(35,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
