...
call cpu_time(start)
call dran_ini(5000)
open(unit=43,file='PagL324.txt',status='replace',action='write')
...
do rho2=4,4
	...
	open(unit=43,file='PagL324.txt',status='old',position='append',action='write')
	write(43,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=43)
