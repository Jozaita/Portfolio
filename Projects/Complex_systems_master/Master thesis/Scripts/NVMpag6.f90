...
open(unit=76,file='NVMpags6.txt',status='new',action='write')
 close(unit=76)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=76,file='NVMpags6.txt',status='old',position='append',action='write')
	write(76,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=76)
