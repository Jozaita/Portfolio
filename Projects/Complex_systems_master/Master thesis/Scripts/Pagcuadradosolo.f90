...
call cpu_time(start)
call dran_ini(5000)
open(unit=40,file='Pruebi2.txt',status='old',action='write')
...
do rho2=0,0
	...
	open(unit=40,file='Pruebi2.txt',status='old',position='append',action='write')
	write(40,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=40)
