...
real(dp),dimension(50000)::agemaj,agemin
real(dp)::dran_u,start,al2,al4,al5,a,sumaj,sumin,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin,x

call cpu_time(start)
call dran_ini(5000)
open(unit=46,file='PagL327.txt',status='replace',action='write')
...
do rho2=1,4
	...
	open(unit=46,file='PagL327.txt',status='old',position='append',action='write')
	write(46,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=46)
