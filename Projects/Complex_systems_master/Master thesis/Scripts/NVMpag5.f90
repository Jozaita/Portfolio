...
open(unit=75,file='NVMpags5.txt',status='new',action='write')
 close(unit=75)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=75,file='NVMpags5.txt',status='old',position='append',action='write')
	write(75,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=75)
