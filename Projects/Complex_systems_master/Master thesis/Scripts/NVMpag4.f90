...
open(unit=74,file='NVMpags4.txt',status='new',action='write')
 close(unit=74)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=74,file='NVMpags4.txt',status='old',position='append',action='write')
	write(74,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=74)
