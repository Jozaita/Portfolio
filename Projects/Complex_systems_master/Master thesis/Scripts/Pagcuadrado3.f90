...
call cpu_time(start)
call dran_ini(5000)
open(unit=42,file='PagL323.txt',status='replace',action='write')
...
do rho2=3,3
	...
	open(unit=42,file='PagL323.txt',status='old',position='append',action='write')
	write(42,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=42)
