...
open(unit=60,file='NVM9antiag103.txt',status='replace',action='write')
open(unit=80,file='NVM9antiag2103.txt',status='replace',action='write')
write(60,'(4f13.6)')centre(1),centre2(1),histo(1)/((centre(1)-xmin)*dble(sim-1)),histo2(1)/(centre(1)*dble(sim-1))
do i=2,ncol-1
	write(60,'(4f13.6)')centre(i),centre2(i),histo(i)/((centre(i)-centre(i-1))*dble(sim-1)),& 
	histo2(i)/((centre2(i)-centre2(i-1))*dble(sim-1))
	
end do
write(60,'(4f13.6)')centre(ncol),centre2(ncol),histo(ncol)/((xmax-centre(ncol))*dble(sim-1)),histo2(ncol)/(centre(ncol)*dble(sim-1))
