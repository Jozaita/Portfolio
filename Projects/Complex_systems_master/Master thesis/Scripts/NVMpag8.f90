...
open(unit=78,file='NVMpags8.txt',status='new',action='write')
 close(unit=78)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=78,file='NVMpags8.txt',status='old',position='append',action='write')
	write(78,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=78)
