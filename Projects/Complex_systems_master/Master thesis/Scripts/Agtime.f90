...
integer,parameter::ncol=50	 , long=3000
integer(dp),parameter::sim=100
integer(dp)::tray,i,j,nmax,cont,cont2
real(dp)::n,r,xm,xm2,x,q,m,taup,taun
...
!open(unit=40,file='NVM2agtimepruebi.txt',status='replace',action='write')
open(unit=45,file='NVM2ag2timepruebi2.txt',status='replace',action='write')
...
a=10.0d0**(-3.0d0)
	...
	do tray=1,sim-1
		...
		!M1 evolucion temporal
		cont=0
		do while (t.lt.2*10**3)
		
		!cont=cont+1
		!if (mod(cont,10**3).eq.0) then 
		!	cont=0
			
			!write(*,*)tray,t,2.0d0*(n/Num)-1.0d0,nmax,taup,taun
			!write(40,'(2f13.6)')t,2.0d0*(n/Num)-1.0d0
...
do i=1,long
write(45,'(i5,3f13.6)')i,nipm(i)+ninm(i),nipm(i),ninm(i)
end do
