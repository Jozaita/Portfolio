




program MFp
implicit none
integer,parameter::dp=selected_real_kind(15,307)
integer,parameter:: N=2500,sim=100 
integer::i_dran,tray,i,al1,al3,a2,rho2
integer(dp)::t,mc,b,cont,alp
integer,dimension(N)::s,eti 
integer,dimension(N)::age,lista
real(dp),dimension(20000)::agemaj,agemin
real(dp)::dran_u,start,al2,al4,al5,a,sumaj,sumin,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin,x,rho


call dran_ini(5000)
open(unit=40,file='Auto1.txt',status='replace',action='write')

 close(unit=40)
mc=2*10**3

!Initial condition

do rho2=0,0
	rho=1.0d0
	
	do a2=1,40
		call cpu_time(start)
		a=10**(-4.0d0+4.0d0*dble(a2)/40.0d0)
		m=0.0d0
		m2=0.0d0
		m4=0.0d0
		
		do i=1,size(agemaj)
			agemaj(i)=0.0d0
			agemin(i)=0.0d0
		end do
		do tray=1,sim


			do i=1,N
  				if (dran_u().lt.0.5d0)then 
  				else
    					s(i)=1
  					s(i)=0
  				end if
				age(i)=0
  				
			end do
			
			do i=1,N			
				lista(i)=i
			end do
			cont=0
			do while(cont.lt.int((1.0d0-rho)*N))
				b=i_dran(N)
				if (0.ne.lista(b)) then 
					age(b)=-1
					lista(b)=0
					cont=cont+1
				end if
			end do
			
		!Empieza la dinamica
			do t=1,mc*N
		!Escoger agente 
			al1=i_dran(N)
			
		!Escoger free will o social interaction 	
			al2=dran_u()
			if (al2.gt.a) then !social interaction
				al5=dran_u()
				alp=i_dran(N)
				do while (al1.eq.alp)
					alp=i_dran(N)
				end do
				if (al5.lt.dble(age(alp))/(dble(age(al1))+dble(age(alp)))) then
					
					if (age(al1).gt.-1) then 
						if (s(al1).eq.s(alp)) then 
							age(al1)=age(al1)+1		
						else 
							age(al1)=0
						end if
					end if
					s(al1)=s(alp)
				else 	
					if (age(al1).gt.-1) age(al1)=age(al1)+1
				end if
			else !free will
				al4=dran_u()
				if (al4.gt.0.5d0) then 
					if (age(al1).gt.-1) age(al1)=age(al1)+1 
				else 
					if (age(al1).gt.-1) age(al1)=0
				s(al1)=abs(1-s(al1))
				end if
			end if
			!if ((mod(t,10**2*N).eq.0).and.(tray.eq.1)) then 
			!	write(20,'(i13,f13.6)')t,2*sum(s)/dble(N)-1.0d0
			!end if	
			end do
			x=dble(sum(s))
			mtemp=2*x/dble(N)-1.0d0
			
			m=m+abs(mtemp)
			m2=m2+mtemp**2.0d0
			m4=m4+mtemp**4.0d0	
			if (mtemp.gt.0.0d0) then 
				do i=1,N
					if (age(i).gt.0) then
						if (s(i).eq.1) agemaj(age(i)+1)=agemaj(age(i)+1)+1.0d0
						if (s(i).eq.0) agemin(age(i)+1)=agemin(age(i)+1)+1.0d0
					end if
				end do			
			else if (mtemp.lt.0.0d0) then
				do i=1,N 
					if (age(i).gt.0) then
						if (s(i).eq.0) agemaj(age(i)+1)=agemaj(age(i)+1)+1.0d0
						if (s(i).eq.1) agemin(age(i)+1)=agemin(age(i)+1)+1.0d0
				        end if 
				end do	
			end if
		
		!write(*,*)tray,a,mtemp,finish-start
		
		end do
	taumaj=0.0d0
	taumin=0.0d0
	m=m/dble(sim)
	m2=m2/dble(sim)
	m4=m4/dble(sim)
	sumaj=sum(agemaj)
	sumin=sum(agemin)
	do i=1,size(agemaj)	
	taumaj=taumaj+agemaj(i)*dble(i-1)/sumaj
	taumin=taumin+agemin(i)*dble(i-1)/sumin
	end do
	
	m4=1.0d0-m4/(3.0d0*m2**2.0d0)
	m2=(m2-m**2.0d0)*N
	
	call cpu_time(finish)
	open(unit=40,file='Auto1.txt',status='old',position='append',action='write')
	write(40,'(8f13.6)')a,rho,m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)a,rho,m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)finish-start
	close(unit=40)
	
	end do
write(40,'(a5)')" "	
end do

end program 


