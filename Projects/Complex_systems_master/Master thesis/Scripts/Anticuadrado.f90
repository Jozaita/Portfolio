...
real(dp)::dran_u,start,al2,al4,al5,a,x,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin


call dran_ini(5000)
open(unit=10,file='AntiL40.txt',status='replace',action='write')
!open(unit=20,file='L8time.txt',status='replace',action='write')
 close(unit=10)
mc=10**4
...
do a2=1,30
	a=10.0d0**(-3.0d0+3.0d0*dble(a2)/30.0d0)
	m=0.0d0
	m2=0.0d0
	m4=0.0d0
	do i=1,size(agemaj)
	...
	!Escoger free will o social interaction 	
		...
		if (al2.gt.a) then !social interaction
			al5=dran_u()
			if (al5.gt.1.0d0/(dble(age(al1))+2.0d0)) then
				neigh(1)=n1(al1)
				neigh(2)=n2(al1)
				neigh(3)=n3(al1)
				neigh(4)=n4(al1)
	...
	open(unit=10,file='AntiL40.txt',status='old',position='append',action='write')
	write(10,'(7f13.6)')a,m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=10)
	write(*,*)a,m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)finish-start
end do	
end program 
