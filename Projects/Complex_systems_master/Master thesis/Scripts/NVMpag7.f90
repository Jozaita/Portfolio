...
open(unit=77,file='NVMpags7.txt',status='new',action='write')
 close(unit=77)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=77,file='NVMpags7.txt',status='old',position='append',action='write')
	write(77,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=77)
