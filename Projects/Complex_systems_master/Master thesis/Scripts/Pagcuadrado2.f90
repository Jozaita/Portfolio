...
real(dp),dimension(15000)::agemaj,agemin
real(dp)::dran_u,start,al2,al4,al5,a,sumaj,sumin,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin,x

call cpu_time(start)
call dran_ini(5000)
open(unit=71,file='PagL322casa.txt',status='replace',action='write')
...
do rho2=2,2
	...
	open(unit=71,file='PagL322casa.txt',status='old',position='append',action='write')
