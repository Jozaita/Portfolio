...
open(unit=25,file='NVMpagn92.txt',status='new',action='write')
 close(unit=25)
	
apasos=20
do a2=1,10
a=0.05d0+dble(a2)*5.0d0*10.0d0**(-3.0d0)
do rho2=1,10
	call cpu_time(start)
	xm=0.0d0
	xm2=0.0d0
	...
	rho=0.90d0+dble(rho2)*10.0d0**(-2.0d0)
	tmax=2*10**(3)	
	do tray=1,sim
	...
	call cpu_time(finish)
	open(unit=25,file='NVMpagn92.txt',status='old',position='append',action='write')
	write(25,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
