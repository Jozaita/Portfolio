...
program Pagcuadradocasa2
implicit none
integer,parameter::dp=selected_real_kind(15,307)
integer,parameter:: L=32
integer,parameter:: N=L*L,sim=int(N/10)
...
real(dp),dimension(10000)::agemaj,agemin
real(dp)::dran_u,start,al2,al4,al5,a,sumaj,sumin,finish
real(dp)::mtemp,m,m2,m4,taumaj,taumin,x


call dran_ini(5000)
open(unit=68,file='PagL32casa1p2.txt',status='replace',action='write')
...
do rho2=6,10
	...
	open(unit=68,file='PagL32casa1p2.txt',status='old',position='append',action='write')
	write(68,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	write(*,*)a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin,finish-start
	close(unit=68)
