...
open(unit=80,file='NVMpags10.txt',status='new',action='write')
 close(unit=80)
...
do rho2=1,10
	...
	rho=0.20d0+dble(rho2)*10.0d0**(-2.0d0)
	tmax=2*10**(3)	
	do tray=1,sim
	...
	call cpu_time(finish)
	open(unit=80,file='NVMpags10.txt',status='old',position='append',action='write')
	write(80,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=80)
