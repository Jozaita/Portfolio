...
real(dp),parameter::Num=1500.0d0,rho=0.20d0
...
open(unit=81,file='NVMpag031.txt',status='new',action='write')
 close(unit=81)
	
apasos=20
do a2=1,10
a=dble(a2)*2.5d0*10.0d0**(-3.0d0)+5.0d0*10.0d0**(-2.0d0)
	...
	call cpu_time(finish)
	open(unit=81,file='NVMpag031.txt',status='old',position='append',action='write')
	write(81,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=81)
