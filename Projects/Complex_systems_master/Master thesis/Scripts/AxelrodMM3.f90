...
integer,parameter:: L=16,N=L*L,F=5
...
integer,parameter::dp=selected_real_kind(15,307)
integer,parameter::  L=16, N=L*L,F=5, L2=1
integer,dimension(4)::neigh
integer,dimension(N)::n1,n2,n3,n4
integer,dimension(N,F)::atr
...
implicit none
integer,parameter::dp=selected_real_kind(15,307)
integer,parameter:: L=16, F=5,sim=50
...
integer,dimension(4)::neigh
real(dp)::start,dens,finish,act,tmaxprom,b,dran_u,mu,tmaxvar,lmaxprom,lmaxvar,rho
real(dp),dimension(100)::smax 

call dran_ini(5000)
open(unit=20,file='AxelrodMM3l14.txt',status='replace',action='write')

!GLOBAL
b=0.5d0
L1=4
rho=(dble(L1)/dble(L))**2.0d0
contador=1
allocate(chosen(L1**2))
do j=-L1/2+1,L1/2
	do i=-L1/2+1,L1/2
		chosen(contador)=dble((N-L))/2-i+j*L+1	
		contador=contador+1
	end do
end do
write(*,*)chosen
...
do while (dens.gt.0.0d0)
	...
	else if (contador2.eq.L1**2) then 
			neigh(1)=n1(suj)
			neigh(2)=n2(suj)
			neigh(3)=n3(suj)
			neigh(4)=n4(suj)
...
write(20,'(i5,4f13.6)')q,tmaxprom,tmaxvar,lmaxprom,lmaxvar
deallocate(freq)
end do
call cpu_time(finish)
write(*,*)finish-start

end program AxelrodMM3
