...
open(unit=79,file='NVMpags9.txt',status='new',action='write')
 close(unit=79)
...
do rho2=1,10
	...
	call cpu_time(finish)
	open(unit=79,file='NVMpags9.txt',status='old',position='append',action='write')
	write(79,'(8e13.6)')a,rho,xm,xm2,xm4,taumaj,taumin,taumaj-taumin
	close(unit=79)
