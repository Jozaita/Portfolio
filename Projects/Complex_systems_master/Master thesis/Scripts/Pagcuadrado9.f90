...
call cpu_time(start)
call dran_ini(5000)
open(unit=48,file='PagL329.txt',status='replace',action='write')
...
do rho2=9,9
	...
	open(unit=48,file='PagL329.txt',status='old',position='append',action='write')
	write(48,'(8f13.6)')a,dble(rho)/dble(N),m,m2,m4,taumaj,taumin,taumaj-taumin
	close(unit=48)
